...
func (s *kvsServer) Add(ctx context.Context, r *pd.Entry) (*pd.AddResponse, error){
	s.mu.Lock()
	defer s.mu.Unlock()
	if val, ok := s.elements[r.Key]; ok {
	...
	return &pb.AddResponse{}, grpc.Errorf(codes.NotFound, "element not found value=[%s]", r.Key)
}
